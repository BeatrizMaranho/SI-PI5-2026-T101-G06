...
config_yaml = './dataset/data.yaml' 

model = YOLO(weights_path)

def exibir_metricas_validacao():
    if os.path.exists(config_yaml):
        print("\nCalculando métricas de validação atuais... aguarde.")
        ...
        map50 = results_dict.get('metrics/mAP50(M)', None)
        map95 = results_dict.get('metrics/mAP50-95(M)', None)
        
        if map50 is not None:
            print(f"mAP50 (Mask): {map50:.4f}")
        if map95 is not None:
            print(f"mAP50-95 (Mask): {map95:.4f}")
        print("="*60)
        ...
        seg_metrics = getattr(metrics, 'seg', None)
        if seg_metrics is not None and hasattr(seg_metrics, 'ap_class_index'):
            print(f"\nPrecision média (Mask): {getattr(seg_metrics, 'mp', 0.0):.4f}")
            print(f"Recall média (Mask): {getattr(seg_metrics, 'mr', 0.0):.4f}")
            print("\nDetalhes por classe:")
            
            # Aqui pegamos os índices reais das classes retornadas na validação
            for i, class_idx in enumerate(seg_metrics.ap_class_index):
                class_name = metrics.names[class_idx]
                p_class = seg_metrics.p[i] if hasattr(seg_metrics, 'p') else 0.0
                r_class = seg_metrics.r[i] if hasattr(seg_metrics, 'r') else 0.0
                print(f"\n  {class_name}:")
                print(f"    Precision: {p_class:.4f}")
                print(f"    Recall: {r_class:.4f}")
        else:
            print("Não foi possível acessar a estrutura 'metrics.seg'.")
        print("="*60 + "\n")
    else:
        print(f"\nErro: O arquivo de configuração '{config_yaml}' não foi encontrado.")
        print("Certifique-se de que a pasta 'dataset' com o 'data.yaml' está no mesmo diretório do script.")
